// ============================================================================
// mall-ui 前端 Jenkins 流水线（构建 dist + rsync over ssh 部署到 mall nginx）
//
...
pipeline {
    ...
    parameters {
        ...
        string(name: 'STATIC_ROOT', defaultValue: '/data/mall/web-ui', description: 'mall nginx 的宿主静态根目录（商城前台部署目标）')
    ...
    stages {
        ...
        stage('构建 dist (node 容器)') {
            steps {
                sh 'mkdir -p /var/lib/jenkins/.cache/mall-ui-pnpm'
                sh """
                  docker run --rm --network host \\
                    -v "\$PWD":/app -w /app \\
                    -v /var/lib/jenkins/.cache/mall-ui-pnpm:/cache \\
